...
Esta é a base do controle de versão. Essas etapas são responsáveis por salvar seu fluxo de trabalho, carregá-lo no repositório do projeto e baixá-lo para o seu computador, ou seja, sincronizar. 
...
Agora, como sou o único *proprietário* do repositório, sou o único com o poder de *aceitar* seu `Pull Request`. Observe que o repositório agora mostra "1" `Pull request` e quando eu o abro, chego a uma página semelhante à sua. A principal diferença é que posso `*Merge pull request*`.   
...
Para criar uma gagina, vá para a página principal do repositório que você criou (não `StudyGroup`). Vá em *Configurações* e clique em *Páginas* no menu esquerdo (o último). Em `Fonte` selecione "principal". Serão apresentadas duas opções:
