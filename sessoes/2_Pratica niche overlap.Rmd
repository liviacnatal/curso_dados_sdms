...
Além do ecospat, usaremos `terra` ([Hijmans et al. 2024](https://CRAN.R-project.org/package=terra)), para manipular os dados espaciais, `geodata` para baixar WorldClim ([Fick and Hijmans 2017](https://doi.org/10.1002/joc.5086)) rasters e; `rgbif` ([Chamberlain et al. 2021](https://CRAN.R-project.org/package=rgbif)) para baixar registros GBIF, `maptools` ([Bivand e Lewin-Koh 2021](https://CRAN.R-project.org/package=maptools)) para obter um mapa base mundial para gráficos e `ade4` ([Dray et al. 2024](https://adeverse.github.io/ade4/)) para calcular PCAs.
...
pjOccs <- vect(pjOccs, geom = c("decimalLongitude",
                                "decimalLatitude"))
...
points(pjOccs, pch = 16, col = 2, cex = 0.3)
...
```{r}
pjOccs <- cbind(pjOccs, terra::extract(wclim,pjOccs))
```
...
```{r}
pjOccs <- pjOccs[complete.cases(data.frame(pjOccs)), ]
```

# Dividindo Dados

Neste ponto, todos os dados que precisamos para a análise de Quantificação de Nicho estão em pjOccs e wclimMat. Precisamos dividir esses dados em regiões nativas e invasivas para nossa comparação. Vamos nos restringir ao continente Americano, e considerar todos os registros entre 120 e 60 graus oeste, e entre 10 graus ao sul a 40 graus ao Norte como nativos. Consideramos todos os registros do Brasil como invasivos.
...
```{r}
## Crop Climate Layers:
naEnvR <- crop(wclim, pj_native_area)
invEnvR <- crop(wclim, pj_invasive_area)

## Extract values to matrix:
naEnvM <- values(naEnvR)
invEnvM <- values(invEnvR)
...
globalEnvM <- rbind(naEnvM, invEnvM)
```
...
pj.native.scores <-
  suprow(pca.clim,
         data.frame(pj_native)[, colnames(globalEnvM)])$li   
pj.invasive.scores <-
  suprow(pca.clim,
         data.frame(pj_invasive)[, colnames(globalEnvM)])$li
...
Vamos analisar isso. `dudi.pca` faz uma análise PCA em `globalEnvM`, que é uma matriz de todas as variáveis ambientais em toda a área de estudo. Usamos isso para criar um resumo bidimensional da variabilidade ambiental total.

Em seguida, mapeamos nossos dados de observação (`pj_native` e `pj_invasive`) nessa ordenação bidimensional, usando a função `suprow`. `pj_native` e `pj_invasive` são objetos `SpatVector`. Às vezes, você pode tratá-los como se fossem `data.frames`, mas outras vezes você precisa convertê-los explicitamente. Esta é uma dessas vezes, por isso os envolvi em `data.frame()`.

Lembre-se de que `pj_native` e `pj_invasive` têm mais colunas do que a matriz ambiental: eles também incluem ano, countryCode, país. Queremos incluir apenas as variáveis ambientais quando você projeta as observações na ordenação. Para garantir que usamos as mesmas variáveis da ordenação original de `globalEnvM`, na mesma ordem, seleciono as colunas explicitamente para corresponder a esse objeto:

```{r}
head(data.frame(pj_native)[, colnames(globalEnvM)])
```
...
ecospat.plot.niche.dyn(nativeGrid, invasiveGrid, intersection = 0.05) 
```
...
pjInvearly <- subset(pj_invasive, pj_invasive$year <= 1990)
pjInvlate <- subset(pj_invasive, pj_invasive$year > 1990)
...
earlyGeoGrid <- ecospat.grid.clim.dyn(geoGrid, geoGrid,
                                      terra::geom(pjInvearly)[,c(3,4)])

lateGeoGrid <- ecospat.grid.clim.dyn(geoGrid, geoGrid,
                                     terra::geom(pjInvlate)[,c(3,4)])
